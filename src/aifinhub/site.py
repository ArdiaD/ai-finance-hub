...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  .abs {{ color:#c7ccd6; font-size:14px; }}
...
function card(p){{
  ...
  return `<div class="card${{feat}}">
    ...
    <p class="abs">${{esc((p.abstract||'').slice(0,400))}}${{(p.abstract||'').length>400?'…':''}}</p>
    <div class="links"><a href="${{esc(p.url)}}" target="_blank" rel="noopener">Link</a>${{pdf}}</div>
  </div>`;
}}
function render(){{
  const q=el('q').value.toLowerCase(), src=el('src').value, sort=el('sort').value;
...
fetch('papers.json').then(r=>r.json()).then(d=>{{
  ...
  ['q','src','sort'].forEach(id=>el(id).addEventListener('input',render));
  render();
}});
</script>
</body>
