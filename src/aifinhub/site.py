...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  .hubhead {{ position:relative; overflow:hidden; }}
  .hubhead::before {{ content:""; position:absolute; inset:0; z-index:0;
     background:url("../hero-bg.jpg") center right / cover no-repeat; }}
  .hubhead::after {{ content:""; position:absolute; inset:0; z-index:1;
     background:linear-gradient(180deg,transparent 48%,var(--bg) 100%),
                linear-gradient(90deg,var(--bg) 12%,rgba(10,14,28,.72) 44%,
                  rgba(10,14,28,.28) 78%,rgba(10,14,28,0) 100%); }}
  header {{ position:relative; z-index:2; max-width:1100px; margin:0 auto; padding:64px 24px 44px; }}
